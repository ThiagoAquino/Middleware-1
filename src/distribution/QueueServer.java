...
/*
	public void doingTimeToLiveThread(){
		new Thread(){
			public void run(){
				try {
 ...
 */
/*
	public void doingTimeToLive(){

		long timeToLive = (long) QueueManager.getInstance().getTimeToLive();
		...
		for (String chave : keys) {
			...
			for(int i = 0; i < queue.size();i++){
				...
				long totalTime = (currentTime - msg.getTimeOfArrival()) / 1000;
